...
dorothea = access_ga(ga_property_list$dorothea)
```
...
```{r "funki-stats", eval=FALSE, fig.asp=0.9, include=FALSE}
funki = access_ga(ga_property_list$`FUNKI App`)
...
((ts + cs) / m) + plot_annotation(tag_levels = "A") +
  labs(caption = str_c("as of ", today()))
```
...
```{r "ceviche-stats", eval=FALSE, include=FALSE}
ceviche = access_ga(ga_property_list$CEVIChE)
...
```{r "reheat-stats", include=FALSE, eval = FALSE}
reheat = access_ga(ga_property_list$ReHeat)
...
```{r "ckd-stats", eval=FALSE, include=FALSE}
ckd = access_ga(ga_property_list$`CKD App`)
...
progeny = access_ga(ga_property_list$progeny)
